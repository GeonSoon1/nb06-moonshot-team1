...
import { CachedGoogleAccessToken } from '../types/oAuth';
...
async function refreshGoogleAccessToken(refreshToken: string): Promise<{
  ...
  const resRefresh = await fetch('https://oauth2.googleapis.com/token', {
    method: 'POST',
  ...
  if (!resRefresh.ok) {
    ...
    throw new UnauthorizedError(`Google refresh failed`);
  } //구글 연동 실패니 다시 로그인하라!

  // { access_token, expires_in, scope?, token_type? }
  if (!json.access_token || typeof json.expires_in !== 'number') {
    throw new UnauthorizedError('Google refresh response missing fields');
...
async function setCacheGoogleToken(userId: number, cacheKey: string): Promise<string> {
  ...
  if (!account?.refreshTokenEnc) {
    throw new UnauthorizedError('Google not connected (missing refresh token)');
  }

  const refreshToken = decryptToken(account.refreshTokenEnc);
  if (!refreshToken) {
    throw new UnauthorizedError('Refresh token decrypt failed');
  }

  const { accessToken, expiresInSec } = await refreshGoogleAccessToken(refreshToken);
  ...
  googleAccessTokenCache.set(
    cacheKey,
    { token: accessToken, expiresAtMs },
    { ttl: Math.max(1, expiresInSec - 60) * 1000 } // 실제 값보다 60초 일찍 캐시에서 만료 처리, 네트워크 지연이나 서버 시간 오차 등의 이유로
  );
