...
- **Documentation**: **REST Client**, **Swagger**
...
- **정교한 멤버 초대 워크플로우**
  - UUID 기반 고유 초대 링크 발급 및 상태 관리(Pending/Accepted/Rejected/Canceled/Quit)
  - 멤버 목록 내 초대 대기 상태 실시간 확인 및 소유자 권한의 초대 취소/멤버 제외
  - 트렌젝션을 활용한 Invitation과 ProjectMember 데이터의 일관성 확보
- **역할 기반 접근 제어(RBAC)**
  - 리소스 계층(SubTask -> Task -> Project)을 역추적하는 **resolveProjectId** 로직 개발
  - `projectOwner`, `projectMember` 등 도메인별 전용 권한 인가 미들웨어 구축
